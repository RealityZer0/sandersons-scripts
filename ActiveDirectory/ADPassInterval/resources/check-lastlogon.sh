...
domain="domain.com"
lastLogon="$(/usr/bin/dscl "/Active Directory/${domain}/${domain}" -read /Computers/${adComputer} | awk '/lastLogonTimestamp/ {print $2}')"
lastLogonCvrt=$((lastLogon/10000000-11644473600))
lastLogonDate="$(/bin/date -j -f %s ${lastLogonCvrt})"
todayEpoch="$(/bin/date +%s)"
objChange="$(/usr/bin/dscl "/Active Directory/${domain}/${domain}" -read /Computers/${adComputer} | awk '/whenChanged/ {print $2}'| cut -c 1-14)"
