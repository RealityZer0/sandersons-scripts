...
FV2InsKey="/Library/Keychains/FileVaultMaster.keychain"
...
main() {
  ...
  if [[ $1 == "root" || $1 == "macadmin" ]]; then
    log "Exiting Crypt hook for $1 logging in."
    exit 0
  fi
  
# Check for and remove existing Institutional key
  if [ -e $FV2InsKey ]; then
	/bin/rm -f $FV2InsKey
	else
	echo "No Institutional key found"
  fi
